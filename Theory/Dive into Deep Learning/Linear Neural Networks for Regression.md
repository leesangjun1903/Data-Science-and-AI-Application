...
딥 뉴럴 네트워크는 실제로는 잘 일반화되지만, 훈련 오류만으로는 많은 것을 결론지을 수 없을 정도로 강력합니다. 이러한 경우에는 보류 데이터에 더 많이 의존하여 이후 일반화를 인증해야 합니다.  
보류 데이터, 즉 검증 세트에 대한 오류를 검증 오류라고 합니다.

### 3.6.2 Underfitting or Overfitting?
훈련 오류와 검증 오류를 비교할 때는 두 가지 일반적인 상황을 염두에 두고자 합니다.  
첫째, 훈련 오류와 검증 오류가 모두 크지만 둘 사이에 약간의 차이가 있는 경우를 주의해야 합니다.  
모델이 훈련 오류를 줄이지 못하면 모델이 너무 단순하여(즉, 표현력이 부족하여) 모델링하려는 패턴을 포착하지 못할 수 있습니다.  
또한 훈련 오류와 일반화 오류 사이의 일반화 격차(𝑅 EMP-𝑅)가 작기 때문에 더 복잡한 모델을 사용할 수 있다고 믿을 만한 이유가 있습니다. 이 현상을 과소적합이라고 합니다.  
반면 위에서 설명한 것처럼 훈련 오류가 검증 오류보다 현저히 낮아 심각한 과적합을 나타내는 경우를 주의해야 합니다. 

#### Polynomial Curve Fitting
고차 다항식 함수는 저차 다항식 함수 t보다 더 복잡합니다.  
왜냐하면 고차 다항식은 더 많은 매개변수를 가지고 있고 모델 함수의 선택 범위가 더 넓기 때문입니다.  
훈련 데이터셋을 수정할 때, 고차 다항식 함수는 항상 저차 다항식에 비해 더 낮은 (최악의 경우 동일한) 훈련 오차를 달성해야 합니다.  
사실, 각 데이터 예제가 서로 다른 𝑥 값을 가질 때마다, 데이터 예제의 수와 같은 차수를 가진 다항식 함수는 훈련 세트에 완벽하게 맞출 수 있습니다.  
우리는 그림 3.6.1에서 다항식 차수(모델 복잡성)와 과소적합 및 과적합 간의 관계를 비교합니다.

<img width="703" alt="스크린샷 2025-04-24 오후 5 03 41" src="https://github.com/user-attachments/assets/4dd1854c-9c87-4ec0-a4d1-74b908e42809" />

#### Dataset Size
위의 경계에서 이미 알 수 있듯이, 염두에 두어야 할 또 다른 중요한 고려 사항은 데이터셋 크기입니다.  
모델을 고정할 때, 학습 데이터셋에 있는 샘플 수가 적을수록 과적합이 발생할 가능성이 더 높고, 그 심각도도 더 높습니다.  
학습 데이터량을 늘리면 일반화 오차는 일반적으로 감소합니다.  
게다가, 일반적으로 데이터가 많다고 해서 손해가 있는 것은 아닙니다. 고정된 작업과 데이터 분포의 경우, 모델 복잡도는 데이터량보다 더 빠르게 증가해서는 안 됩니다.  
데이터가 많을수록 더 복잡한 모델을 적합시키려고 시도할 수 있습니다. 데이터가 충분하지 않으면, 더 단순한 모델을 이기기 어려울 수 있습니다.  
많은 작업에서 딥러닝은 수천 개의 학습 예제가 있을 때에만 선형 모델보다 우수한 성능을 보입니다.  
부분적으로, 딥러닝의 현재 성공은 인터넷 기업, 저렴한 저장 장치, 커넥티드 기기, 그리고 경제의 광범위한 디지털화로 인해 생겨난 방대한 데이터셋의 풍부함에 크게 기인합니다.

### 3.6.3 Model Selection
일반적으로 우리는 다양한 방식(서로 다른 아키텍처, 학습 목표, 선택된 특성, 데이터 전처리, 학습률 등)으로 서로 다른 여러 모델을 평가한 후에야 최종 모델을 선택합니다.  
여러 모델 중에서 선택하는 것을 모델 선택이라고 부르는 것이 적절합니다 .

원칙적으로 모든 하이퍼파라미터를 선택하기 전까지는 테스트 세트에 손을 대지 않아야 합니다. 모델 선택 과정에서 테스트 데이터를 사용할 경우, 테스트 데이터에 과적합될 위험이 있습니다. 

테스트 세트에서 학습 문제를 해결하는 일반적인 방법은 데이터를 세 가지 방식으로 분할하고, 학습 데이터 세트 와 테스트 데이터 세트 외에도 검증 세트를 포함하는 것입니다.  
하지만 그 결과, 검증 데이터와 테스트 데이터의 경계가 걱정스러울 정도로 모호한 모호한 상황이 발생합니다.  
달리 명시적으로 언급하지 않는 한, 이 책의 실험에서는 실제 테스트 세트 없이, 학습 데이터와 검증 데이터라고 부르는 데이터를 실제로 사용하고 있습니다. 따라서 이 책의 각 실험에서 보고된 정확도는 실제 테스트 세트 정확도가 아닌 검증 정확도입니다.

#### Cross-Validation
훈련 데이터가 부족할 때, 적절한 검증 집합을 구성하기에 충분한 데이터를 보유할 여유가 없을 수도 있습니다.  
이 문제에 대한 인기 있는 해결책 중 하나는 𝐾-폴드 교차 검증을 사용하는 것입니다.  
여기서 원본 훈련 데이터는 겹치지 않는 𝐾 하위 집합으로 나뉩니다.  
그런 다음 모델 훈련과 검증을 𝐾 번 실행하여 매번 𝐾-1 하위 집합에서 훈련하고 다른 하위 집합(해당 라운드의 훈련에 사용되지 않은 하위 집합)에서 검증합니다.  
마지막으로, 훈련과 검증 오류는 𝐾 실험 결과를 평균 내어 추정됩니다.

## 3.7 Weight Decay
첫 번째 정규화 기법을 소개하겠습니다. 더 많은 훈련 데이터를 수집하면 과적합을 완화할 수 있다는 점을 기억하세요.  
하지만 이는 비용이 많이 들거나 시간이 오래 걸리거나, 또는 우리가 통제할 수 없는 영역이어서 단기적으로는 불가능할 수 있습니다.  
지금은 자원이 허용하는 한 고품질 데이터를 충분히 확보했다고 가정하고, 데이터셋이 주어진 상황에서 우리가 활용할 수 있는 도구에 집중할 수 있습니다.

### 3.7.1 Norms and Weight Decay
간단한 해석 중 하나는 선형 함수 𝑓(x)= w ⊤x의 복잡성을 가중치 벡터의 어떤 노름(예: ∥w ∥2)으로 측정하는 것일 수 있습니다.  
우리는 2.3.11절에서 보다 일반적인 ℓ𝑝 노름의 특수한 경우인 ℓ2 노름과 ℓ1 노름을 도입했음을 기억하세요.  
작은 가중치 벡터를 보장하는 가장 일반적인 방법은 손실을 최소화하는 문제에 노름을 페널티 항으로 추가하는 것입니다.  
따라서 우리는 원래의 목표인 훈련 레이블의 예측 손실을 최소화하는 새로운 목표로 대체하여 예측 손실과 페널티 항의 합을 최소화합니다.  
이제 우리의 가중치 벡터가 너무 커지면, 우리의 학습 알고리즘은 훈련 오류를 최소화하는 대신 가중치 노름 ∥w ∥2를 최소화하는 데 초점을 맞출 수 있습니다.  
이것이 바로 우리가 원하는 것입니다. 코드로 설명하기 위해, 우리는 선형 회귀에 대해 3.1절의 이전 예제를 되살립니다. 거기서 우리의 손실은 다음과 같이 주어졌습니다

$$L(\mathbf{w}, b) = \frac{1}{n}\sum_{i=1}^n \frac{1}{2}\left(\mathbf{w}^\top \mathbf{x}^{(i)} + b - y^{(i)}\right)^2.$$

x(𝑖)는 특징이고, 𝑦(𝑖)는 데이터 예제 𝑖의 레이블이며, (w, 𝑏)는 가중치와 바이어스 매개변수라는 점을 기억하세요.  
가중치 벡터의 크기를 페널티화하려면 손실 함수에 ∥w ∥2를 어떻게 추가해야 하지만, 이 새로운 가산 페널티에 대해 모델이 표준 손실을 어떻게 상쇄해야 할까요?  
실제로 우리는 검증 데이터를 사용하여 이 트레이드오프를 비음수 하이퍼파라미터인 정규화 상수 𝜆을 통해 특성화합니다:

$$L(\mathbf{w}, b) + \frac{\lambda}{2} \|\mathbf{w}\|^2.$$

(3.1.11)의 동일한 표기법을 사용하여 ℓ2 정규화 회귀에 대한 미니배치 확률적 경사 하강법을 다음과 같이 업데이트합니다:
