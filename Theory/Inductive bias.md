...
```
오컴의 면도날은 "같은 현상을 설명하는 두 이론이 있다면, 더 단순한 쪽을 선택하라"는 원리로, 14세기 영국 논리학자 오컴의 윌리엄이 제안한 철학적 접근법입니다.
이는 불필요한 가정을 하지 말라는 뜻으로, 과학적 사고에서 경제성, 단순성의 원리로 널리 활용됩니다.

즉, 여러 설명 중 가장 적은 가정과 논리로 현상을 설명할 수 있는 것이 가장 타당하다고 보는 사고 절약의 원리입니다.
예를 들어, 누군가가 지각했을 때 복잡한 이유보다 "늦게 일어났다"는 단순한 이유를 우선시하는 것입니다. 현대 과학에서도 오컴의 면도날은 과도한 복잡성을 줄이고 설명의 효율성을 높이는 데 적용됩니다.
```
