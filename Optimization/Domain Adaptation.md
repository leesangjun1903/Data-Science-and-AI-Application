...
Domain Adaptation의 핵심은 소스 도메인 $$\mathcal{D}_s = \{(x_i^s, y_i^s)\}_{i=1}^{n_s}$$에서 학습된 모델을 타겟 도메인 $$\mathcal{D}_t = \{x_j^t\}_{j=1}^{n_t}$$에 적용할 때 발생하는 분포 차이를 최소화하는 것입니다[3]. 여기서 소스 도메인은 레이블이 있는 데이터이고, 타겟 도메인은 일반적으로 레이블이 없는 데이터입니다[1][4].
